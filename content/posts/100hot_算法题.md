...
>  3.22蚂蚁笔试题目02：平衡码问题，也是使用前缀和变体解决的思路。

<img src="https://picgo-lenblog.oss-cn-beijing.aliyuncs.com/lenblog/202603231407022.png" alt="image-20260323140710125" style="zoom:50%;" />

解题思路：

**数学规律：**

观察0-9发现数学规律：平衡码和非平衡码是交替的。

如果是奇数，每个尾数为0-9的区间，都是平衡和不平衡对半分，最后一个区间也有偶数个元素，也是对半分，因此总共是（x+1）/2平衡的个数(包含0）。如果是偶数，就单独判断当前的数是否是平衡码，前面的奇数个按照奇数算。

**DP：**

核心思想：按位递归填数 + 记忆化缓存，将 “遍历所有数字” 的指数级时间，降为 “遍历唯一状态” 的线性级时间，避免重复计算；

**状态设计（唯一场景）**：固定模板 = 位序（pos）+ 核心统计状态（如本题的数位和奇偶性 parity）+ 限制状态（limit），三个维度唯一决定后续填数规则和统计结果；

- pos：当前填到第几位，保证按位遍历不重不漏；
- parity：当前数位和的奇偶性，对应本题的统计目标；
- limit：当前位是否受原数限制（1 = 受限，0 = 不受限），保证填出的数≤原数 x；

**缓存价值**：缓存存储 “唯一场景的统计结果”，同一 x 的不同填数分支走到同一场景时，直接复用结果，无需重复遍历；

**关键操作**：

- 递归终止：所有位填完后，根据核心状态（parity）判断是否为目标数；
- 状态传递：遍历当前位所有可能数字，计算新的 limit（是否继续受限）和核心状态，递归累加结果；
- 缓存命中：已计算的状态直接返回，未计算则计算后存入缓存。

```java
//数学规律
-----------------------------------------------------------------------------------------
//    判断一个数是不是平衡码
    private int isBanlanceCode(long num){
    ...
    //计算0-x之间的平衡码个数，数学规律:0-9平衡和不平衡是和奇偶一样交替的。
    //如果是奇数，每个尾数为0-9的区间，都是平衡和不平衡对半分，最后一个区间也有偶数个元素，也是对半分，因此总共是（x+1）/2平衡的个数(包含0）
    private long countBalanced(long num){
        if(num==0){
            return 1;
        }
        if (num%2!=0){
            ...
            //如果是偶数,前面就按奇数处理，并加上最后x的结果
            return num/2 + isBanlanceCode(num);
        }
    }
...
-----------------------------------------------------------------------------------------
    //定义记忆缓存：描述当前位的情况
    long[][][] dp;
    //使用数位DP去求0-x有多少平衡码，拆分位数，一位一位地去填，统计数位之和状态
    private long countBalanced2(long num){
        if(num==0){
            return 1;
        }
        String str = String.valueOf(num);
       ...
       return dfsBalanceCode(0,0,1,digit);
    }
    ...
    /**
     * @param pos    当前要填写的位置
     * @param parity 当前填写之前数位之和是否能%2（0就是偶数）
     * @param limit  当前填写能否无限制填（0-9）
     * @param digit  待检测的数字X
     * @return 所有位置填完之后统计出来的平衡码的个数
     */
    private long dfsBalanceCode(int pos,int parity, int limit,int[]digit){
        if(pos==dp.length){
            return parity==0?1:0;
        }
        if (dp[pos][parity][limit]!=-1){
            return dp[pos][parity][limit];
        }
        long res = 0;
        int maxDigit = limit == 1 ? digit[pos] : 9; // 当前位能填的最大数字
        for (int i=0;i<=maxDigit;i++){
            // 计算新的限制：上一轮受限 + 当前填了最大值 → 下一轮受限
            int newLimit = (limit == 1 && i == maxDigit) ? 1 : 0;
            // 计算新的奇偶性：当前奇偶 + 当前数字，取模2
            int newParity = (parity + i) % 2;
            res += dfsBalanceCode(pos + 1, newParity, newLimit, digit);
        }
        dp[pos][parity][limit] = res;
        return res;
    }
-----------------------------------------------------------------------------------------
    //3.22蚂蚁笔试
    //01平衡码问题,寻找区间[l,r]之间的"数位之和为偶数"的元素个数
    public void banlanceCode(String []args){
        //需要写输入
        ...
        for (int i=0;i<total;i++){
            ...
            //解决问题：统计区间之间的平衡码个数
//            方法1：使用数学规律做法
            long res = countBalanced(right) - countBalanced(left-1);

//            方法2：数位DP
            long resDP = countBalanced2(right) - countBalanced2(left-1);
...
| 方法类型   | 核心时间复杂度（单个数 x）            | 扩展到 T 组测试用例 | 关键说明                                                     |
| ---------- | ------------------------------------- | ------------------- | ------------------------------------------------------------ |
| 数学规律法 | O (log₁₀x)（或 O (k)，k 是 x 的位数） | O(T×log₁₀x)         | 核心操作是遍历 x 的数位（如计算数位和），位数 k 与 x 的关系为 k=log₁₀x，常数级效率（如 18 位仅需 18 次操作） |
| 数位 DP 法 | O (log₁₀x)（或 O (k)）                | O(T×log₁₀x)         | 核心是计算 k×2×2=4k 个唯一状态（k 为位数），每个状态遍历最多 10 次，仍为位数级线性复杂度 |
...
这类方法会自动跳过输入中的「空白符」（空格、Tab、换行），直接读取对应类型的数值，读取失败会抛 `InputMismatchException`

一般用的最多的是nextInt（）和nextLong（），看数据范围大于2^31就用long

2.字符串读取方法（按格式选）

| 方法         | 核心规则                                                     | 适用场景                                   |
| ------------ | ------------------------------------------------------------ | ------------------------------------------ |
| `next()`     | 1. 跳过开头空白符；2. 遇到空格 / 换行停止；3. 只返回中间非空白字符串 | 读取不含空格的单个字符串（如用户名、密码） |
| `nextLine()` | 1. 读取从当前光标到换行符的所有内容；2. 包含空格；3. 读取后跳过换行符 | 读取含空格的整行文本（如地址、一句话）     |

混合使用基本类型 + nextLine () 的问题

这是最常见的错误！比如先调用 `nextInt()`，再调用 `nextLine()`，会发现 `nextLine()` 直接跳过输入，原因是：

- `nextInt()` 只读取数值，**不会读取输入后的换行符**（`\n`）；

- 后续的 `nextLine()` 会直接读取这个残留的换行符，返回空字符串。

- **解决方法**：

  在 `nextInt()`/`nextLong()` 等之后，加一个 `sc.nextLine()` 「吃掉」残留的换行符：
...
- 额外过滤：奇偶判断，如果是奇数肯定不可能都配对
- java中初始化栈要用Deque（这里用具体的类ArrayDeque）
- 这里直接pop就行，初始化map也可以用匿名类初始化（不重要）
- 最后栈如果不为空肯定不是配对的
...
需要的结果是单调性的，用辅助栈保存单调结果。辅助的单调栈数量可以不用和主栈保持一致，只需要在关键节点时候进行比较更新保证结果正确。这里获取最小值的时候，只需要保存小于等于当前最小值的元素即可。
...
- 能找到更高温度的元素：会在后续遍历中被弹出栈，更新 `res[j]`；
- 找不到更高温度的元素：会一直留在栈里，`res[j]` 保持默认值 `0`（符合题目要求）；
...
也可以优化成两次遍历，left和right一起求出来。此时right的定义会发生变化，`right[i]` = 右侧第一个**小于等于** `heights[i]` 的下标。

这样代码中可以这样同时求right和left

```java
//right初始化：Arrays.fill(right, n);
...
这样做的原因是，right的定义改变，但是left定义不变，所以如果有两个柱子高度都是最优解时候，右侧的柱子计算出来的面积还是正确的最大的面积的。

#### 递归解码字符串

[394. 字符串解码 - 力扣（LeetCode）](https://leetcode.cn/problems/decode-string/submissions/703442439/?envType=study-plan-v2&envId=top-100-liked)
...
这里巧思在于每次记录初始时候腐烂的句子和提前记录新鲜橘子总数
...
```java
//子集-回溯，遍历当前层的所有选项（可选元素）→ 选一个合法选项 → 递归处理下一层 → 撤销选择（回溯）
    public List<List<Integer>> subsets(int[] nums) {
        List<List<Integer>> res = new ArrayList<>();
        List<Integer> path = new ArrayList<>();
        dfsSubsets(res,path,nums,0);
        return res;
    }
    public void dfsSubsets(List<List<Integer>> res,List<Integer> path ,int[] nums,int start){;
        ...
        //不选当前的元素，直接进入下一层
        dfsSubsets(res,path,nums,start+1);
        //选当前的元素，进入下一层
        path.add(nums[start]);
        dfsSubsets(res,path,nums,start+1);
        if (!path.isEmpty()) {
...
```java
//遍历当前行的所有列（可选选项）→ 选一个合法列 → 递归处理下一行 → 撤销选择（回溯）
...
public void dfsQueens(List<List<String>> res,int r,
    ...
    //判断当前列行不行
         if (!col[c] && !diag1.getOrDefault(r+c,false) && !diag2.getOrDefault(r-c,false)){
            ...
            //回退当前层的状态
            col[c] = false;
            diag1.put(r+c,false);//这里直接remove也可以
            diag2.put(r-c,false);
         }
    }
}
...
>  在子集问题、电话号码排列问题等情况和全排列在回溯写法的区别

**1. 最核心差异：「可选选项范围」决定是否有 for**

- 子集（选 / 不选）：只有 2 个选项，无需 for；
- 电话号码排列：当前位有多个字符可选（如 2→3 个），必须 for 遍历；
- 全排列：有多个未用元素可选，必须 for 遍历。

**2. 第二核心差异：「结果收集时机」（问题目标决定）**

- 子集的目标是 “所有可能的元素组合”（包括空集、单元素、全元素），所以每走一步（每一层）都要收集；
- 电话号码排列 / 全排列的目标是 “完整的排列”（长度必须等于输入长度），所以只有走到头（终止条件）才收集。

**3. 第三核心差异：「防重复逻辑」（问题特性决定）**

- 子集：要避免`[1,2]`和`[2,1]`重复，靠`start`限制 “只往后选”（选 1 后只能选 2，不能回头选 0）；
- 全排列：要避免重复选同一个元素，靠`used`数组标记；
- 电话号码排列：输入是固定顺序（如 “23”≠“32”），靠`idx`按顺序处理每一位，天然不重复。
...
```java
//方法二：答案的视角（枚举子串结束位置）把子集问题的 “选单个元素” 升级为 “选一段合法的连续子串”
// 核心还是「枚举合法选择 → 递归处理剩余 → 回溯试错」的回溯三板斧，只是 “选择单元” 和 “合法性校验” 适配了分割问题的场景。
public List<List<String>> partition(String s) {
    List<List<String>> res = new ArrayList<>();
    List<String> strs = new ArrayList<>();
    dfsPartition(res,strs,s,0);
    return res;
}

public  void dfsPartition(List<List<String>> res,List<String> strs,String s,int start){
    if(start==s.length()){
...
//组合总和-回溯，遍历当前层的所有选项（可选元素）→ 选一个合法选项 → 递归处理下一层 → 撤销选择（回溯）
public List<List<Integer>> combinationSum(int[] candidates, int target) {
    List<List<Integer>> res = new ArrayList<>();
    List<Integer> path = new ArrayList<>();
    Arrays.sort( candidates);
    dfsComninationSum(res,path,candidates,target,0,target);
    return res;
}

public void dfsComninationSum(List<List<Integer>> res,List<Integer> path,int[] candidates,int target,int start,int left){
    if (start==candidates.length || left < 0 ) {
        return;
    }
    if (left==0){
        res.add(new ArrayList<>(path));
    ...
    dfsComninationSum(res,path,candidates,target,start,left-candidates[start]);
    path.remove(path.size()-1);
    //不选这一层，这里注意left不变，因为选元素要再path那里才是选，这里表示从下一个元素开始，跳过当前元素
    dfsComninationSum(res,path,candidates,target,start+1,left);
}
```
...
//括号生成-回溯，遍历当前层的所有选项（可选元素）→ 选一个合法选项 → 递归处理下一层 → 撤销选择（回溯）
public List<String> generateParenthesis(int n) {
    List<String> res = new  ArrayList<>();
    StringBuilder sb = new StringBuilder();
    dfsGenerateParenthesis(res,sb,0,2*n,0);
    return res;
}

public void dfsGenerateParenthesis(List<String> res,StringBuilder sb,int index,int n,int balance){
    // 提前剪枝：balance < 0 或 balance > n（左括号最多n个）
    ...
    sb.append('(');
    balance++;
    dfsGenerateParenthesis(res,sb,index+1,n,balance);
...
```java
//单词搜索-回溯，遍历当前层的所有选项（可选元素）→ 选一个合法选项 → 递归处理下一层 → 撤销选择（回溯）
int [][]DIRECTION = new int [][]{{0,1},{1,0},{0,-1},{-1,0}};
...
public boolean exist(char[][] board, String word) {
    ...
    // 优化一,如果单词中某个字符的数量超过了棋盘上该字符的数量，直接返回 false
...
public boolean dfsExist(char[][] board, String word,int index,int i,int j){
    ...
    if (index==word.length()-1){
        return true;//匹配+是最后一个字符 → 返回true
    }
    char temp = board[i][j];
    board[i][j] = '#';//标记为访问过，防止重复
    for (int[] d:DIRECTION){
        int x = i + d[0];
        int y = j + d[1];
        if (dfsExist(board,word,index+1,x,y)){
            return true;
        }
    }
    board[i][j] = temp;//回退状态
...
```java
    ...
    public int findKthLargest(int[] nums, int k) {
        int left = 0;
        int right = nums.length-1;
        int kthIndex = nums.length-k;//第k大的数的索引
        while (true) {//一定能找到结果所以不用加额外的return
...
> 参数1值为小，参数2值为大，忽略减号，想要（参数1、参数2）还是（参数2、参数1）直接写就行。
>
> 想要小跟堆，小的在前面，于是写a.getValue()-b.getValue()
...
//返回第K个高频的数字 哈希+小跟堆
    public int[] topKFrequent(int[] nums, int k) {
        ...
        //比较器永远，参数1值为小，参数2值为大，忽略减号，想要（参数1、参数2）还是（参数2、参数1）直接写就行
        PriorityQueue<Map.Entry<Integer,Integer>> pq = new PriorityQueue<>(
                (a,b)->a.getValue()-b.getValue()
        );
        //始终维护一个容量是k的小跟堆，一旦超过k就踢出去堆顶那个就可以了，比大跟堆要简单
